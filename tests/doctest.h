...
// ======================================================================
//
// doctest.h - the lightest feature-rich C++ single-header testing framework for unit
// tests and TDD
//
...
//
// The concept of subcases (sections in Catch) and expression decomposition are from
// there. Some parts of the code are taken directly:
...
// https://github.com/martinmoene/lest which uses the Boost Software License - Version 1.0
...
// GCC/Clang and GCC/MSVC are mutually exclusive, but Clang/MSVC are not because of
// clang-cl...
...
// https://en.cppreference.com/w/cpp/compiler_support MSVC C++11 feature support table:
// https://msdn.microsoft.com/en-us/library/hh567368.aspx GCC C++11 feature support table:
// https://gcc.gnu.org/projects/cxx-status.html MSVC version table:
...
// A 24 byte string class (can be as small as 17 for x64 and 13 for x86) that can hold
// strings with length of up to 23 chars on the stack before going on the heap - the last
// byte of the buffer is used for:
// - "is small" bit - the highest bit - if "0" then it is small - otherwise its "1" (128)
// - if small - capacity left before going on the heap - using the lowest 5 bits
// - if small - 2 bits are left unused - the second and third highest ones
// - if small - acts as a null terminator if strlen() is 23 (24 including the null
// terminator)
//              and the "is small" bit remains "0" ("as well as the capacity left") so its
//              OK
// Idea taken from this lecture about the string implementation of facebook/folly -
// fbstring https://www.youtube.com/watch?v=kPR8h4-qZdk
// TODO:
...
class DOCTEST_INTERFACE String {
   ...
   private:
    ...
    struct view  // len should be more than sizeof(view) - because of the final byte for
                 // flags
    {
        char*     ptr;
        size_type size;
...
struct ContextOptions  //! OCLINT too many fields
{
    ...
    bool
        no_line_numbers;  // if source code line numbers should be omitted from the output
...
#ifdef DOCTEST_CONFIG_INCLUDE_TYPE_TRAITS
    template <typename T>
    explicit Approx(const T& value, typename detail::types::enable_if<
...
#ifdef DOCTEST_CONFIG_INCLUDE_TYPE_TRAITS
    template <typename T>
    typename std::enable_if<std::is_constructible<double, T>::value, Approx&>::type
...
// This will check if there is any way it could find a operator like member or friend and
// uses it. If not it doesn't find the operator or if the operator at global scope is
// defined after this template, the template won't be instantiated due to SFINAE. Once the
// template is not instantiated it can look for global operator using normal conversions.
#ifdef __NVCC__
#define SFINAE_OP(ret, op) ret
#else
#define SFINAE_OP(ret, op)                                                               \
    decltype((void)(doctest::detail::declval<L>() op doctest::detail::declval<R>()),     \
             ret{})
#endif
...
DOCTEST_MSVC_SUPPRESS_WARNING_PUSH
// https://stackoverflow.com/questions/39479163 what's the difference between 4018 and
// 4389
DOCTEST_MSVC_SUPPRESS_WARNING(4388)  // signed/unsigned mismatch
DOCTEST_MSVC_SUPPRESS_WARNING(4389)  // 'operator' : signed/unsigned mismatch
DOCTEST_MSVC_SUPPRESS_WARNING(4018)  // 'expression' : signed/unsigned mismatch
// DOCTEST_MSVC_SUPPRESS_WARNING(4805) // 'operation' : unsafe mix of type 'type' and type
// 'type' in operation
...
struct Expression_lhs {
    ...
    // precedence over the comparisons, but the ExpressionDecomposer class uses the left
    // shift operator to capture the left operand of the binary expression...
    DOCTEST_FORBIT_EXPRESSION(Expression_lhs, <<)
    DOCTEST_FORBIT_EXPRESSION(Expression_lhs, >>)
};
...
struct DOCTEST_INTERFACE ExpressionDecomposer {
    ...
    // operator precedence table) but then there will be warnings from GCC about
    // "-Wparentheses" and since "_Pragma()" is problematic this will stay for now...
    ...
    template <typename L, typename types::enable_if<
...
struct DOCTEST_INTERFACE TestCase : public TestCaseData {
    ...
    int m_template_id;   // an ID used to distinguish between the different versions of a
                         // templated test case
    String m_full_name;  // contains the name (only for templated test cases!) + the
                         // template type
...
struct DOCTEST_INTERFACE MessageBuilder : public MessageData {
    ...
    // kept here just for backwards-compatibility - the comma operator should be preferred
    // now
    template <typename T>
    MessageBuilder& operator<<(const T& in) {
    ...
    // the `,` operator has the lowest operator precedence - if `<<` is used by the user
    // then the `,` operator will be called last which is not what we want and thus the
    // `*` operator is used first (has higher operator precedence compared to `<<`) so
    // that we guarantee that an operator of the MessageBuilder class is called first
...
struct DOCTEST_INTERFACE IReporter {
    ...
    // called when a query should be reported (listing test cases, printing the version,
    // etc.)
    virtual void report_query(const QueryData&) = 0;

    // called when the whole test run starts
    virtual void test_run_start() = 0;
    // called when the whole test run ends (caching a pointer to the input doesn't make
    // sense here)
    virtual void test_run_end(const TestRunStats&) = 0;

    // called when a test case is started (safe to cache a pointer to the input)
    virtual void test_case_start(const TestCaseData&) = 0;
    // called when a test case is reentered because of unfinished subcases (safe to cache
    ...
    // called when a test case is skipped either because it doesn't pass the filters, has
    // a skip decorator or isn't in the execution range (between first and last) (safe to
    ...
    // can iterate through contexts which have been stringified automatically in their
    // destructors when an exception has been thrown
...
#define DOCTEST_TEST_CASE_CLASS(decorators)                                              \
    DOCTEST_CREATE_AND_REGISTER_FUNCTION_IN_CLASS(                                       \
        DOCTEST_ANONYMOUS(DOCTEST_ANON_FUNC_), DOCTEST_ANONYMOUS(DOCTEST_ANON_PROXY_),   \
        decorators)
...
#define DOCTEST_TEST_CASE_TEMPLATE(dec, T, ...)                                          \
    DOCTEST_TEST_CASE_TEMPLATE_IMPL(dec, T, DOCTEST_ANONYMOUS(DOCTEST_ANON_TMP_),        \
                                    __VA_ARGS__)
...
#define DOCTEST_WARN_THROWS_WITH(expr, with, ...)                                        \
    [] {                                                                                 \
        static_assert(                                                                   \
            false, "Exception translation is not available when doctest is disabled.");  \
        return false;                                                                    \
    }()
...
#define DOCTEST_EXCEPTION_EMPTY_FUNC                                                     \
    [] {                                                                                 \
        static_assert(false,                                                             \
                      ...
                      "Use DOCTEST_CONFIG_NO_EXCEPTIONS_BUT_WITH_ALL_ASSERTS if you "    \
                      "want to compile with exceptions disabled.");                      \
        return false;                                                                    \
    }()
...
// this is here to clear the 'current test suite' for the current translation unit - at
// the top
...
class MultiLaneAtomic {
   ...
   private:
    // Each thread has a different atomic that it operates on. If more than NumLanes
    // threads use this, some will use the same atomic. So performance will degrade a bit,
    // but still everything will work.
    //
    // The logic here is a bit tricky. The call should be as fast as possible, so that
    // there is minimal to no overhead in determining the correct atomic for the current
    // thread.
    //
    // 1. A global static counter laneCounter counts continuously up.
    // 2. Each successive thread will use modulo operation of that counter so it gets an
    // atomic
    //    assigned in a round-robin fashion.
    // 3. This tlsLaneIdx is stored in the thread local data, so it is directly available
    // with
    //    little overhead.
...
bool operator==(double lhs, const Approx& rhs) {
    ...
    return std::fabs(lhs - rhs.m_value) <
           rhs.m_epsilon *
               (rhs.m_scale + std::max<double>(std::fabs(lhs), std::fabs(rhs.m_value)));
}
...
// reporter with a duplicate name and a different priority but hopefully that won't happen
// often :|
...
// checks if the name matches any of the filters (and can be configured what to do when
// empty)
...
TestCase::TestCase(funcType test, const char* file, unsigned line,
                   const TestSuite& test_suite, const String& type, int template_id) {
    m_file              = file;
    m_line              = line;
...
bool TestCase::operator<(const TestCase& other) const {
    // this will be used only to differentiate between test cases - not relevant for
    // sorting
    if (m_line != other.m_line)
        return m_line < other.m_line;
...
// destroy cannot be inlined into the destructor because that would mean calling stringify
// after ContextScope has been destroyed (base class destructors run after derived class
// destructors). Instead, ContextScope calls this method directly from its destructor.
...
struct FatalConditionHandler {
    static LONG CALLBACK handleException(PEXCEPTION_POINTERS ExceptionInfo) {
        ...
        // message to be printed on the console just once no matter how many threads have
        // crashed.
    ...
    if (rb.m_at & assertType::is_throws) {  //! OCLINT bitwise operator in conditional
        ...
        s << Color::Cyan << assertString(rb.m_at) << "( " << rb.m_expr << ", \""
          ...
          << Color::Cyan << rb.m_exception << "\n";
               ...
               assertType::is_nothrow) {  //! OCLINT bitwise operator in conditional
...
struct ConsoleReporter : public IReporter {
    ...
    // =========================================================================================
    // WHAT FOLLOWS ARE HELPERS USED BY THE OVERRIDES OF THE VIRTUAL METHODS OF THE
    // INTERFACE
    // =========================================================================================
...
#define DOCTEST_DEBUG_OUTPUT_REPORTER_OVERRIDE(func, type, arg)                          \
    ...
    DOCTEST_DEBUG_OUTPUT_REPORTER_OVERRIDE(test_case_exception, const TestCaseException&,
                                           in)
...
bool parseOptionImpl(int argc, const char* const* argv, const char* pattern,
                     String* value) {
    // going from the end to the beginning and stopping on the first occurrence from the
    // end
...
bool parseOption(int argc, const char* const* argv, const char* pattern,
    ...
    if (parseOptionImpl(argc, argv, pattern + strlen(DOCTEST_CONFIG_OPTIONS_PREFIX),
                        value))
        return true;
...
#define DOCTEST_PARSE_AS_BOOL_OR_FLAG(name, sname, var, default)                         \
    if (parseIntOption(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX name "=", option_bool,  \
                       intRes) ||                                                        \
        parseIntOption(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX sname "=", option_bool, \
                       intRes))                                                          \
...
#define DOCTEST_PARSE_INT_OPTION(name, sname, var, default)                              \
    if (parseIntOption(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX name "=", option_int,   \
                       intRes) ||                                                        \
        parseIntOption(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX sname "=", option_int,  \
                       intRes))                                                          \
        p->var = intRes;                                                                 \
    else if (withDefaults)                                                               \
    p->var = default
...
#define DOCTEST_PARSE_STR_OPTION(name, sname, var, default)                              \
    if (parseOption(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX name "=", &strRes,         \
                    default) ||                                                          \
        parseOption(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX sname "=", &strRes,        \
                    default) ||                                                          \
        withDefaults)                                                                    \
    p->var = strRes
...
int Context::run() {
    ...
    // save the old context state in case such was setup - for using asserts out of a
    // testing context
    auto old_cs = g_cs;
    ...
    if (!testArray.empty()) {
        if (p->order_by.compare("file", true) == 0) {
            ...
            // executable with a specific test case in mind - we don't want to slow down
            // the startup times
        }
    }
    ...
    // invoke the registered functions if they match the filter criteria (or just count
    // them)
    for (auto& curr : testArray) {
        ...
        // do not execute the test if we are to only count the number of filter passing
        // tests
        if (p->count)
            continue;
